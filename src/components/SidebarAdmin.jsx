...
import { LayoutDashboard, UserPlus, Users, CreditCard, Bell, MessageCircle, Mail, BarChart3, HelpCircle, Shield, Settings, LogOut, ChevronLeft, ChevronRight, FileText, Stethoscope } from 'lucide-react';
    ...
    >
            ...
            {({ isActive }) => (
              <motion.div
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                  isActive
                    ? 'bg-medcare-purple/10 text-medcare-purple'
      ...
      <div className="p-3 border-t border-gray-100 dark:border-dark-border">
        <div className="flex items-center gap-3 px-3 py-2 mb-2">
          <div className="w-8 h-8 rounded-full bg-medcare-purple/20 flex items-center justify-center text-medcare-purple text-sm font-bold">
            {user?.name?.[0] || 'A'}
          </div>
          {!collapsed && (
            <div className="min-w-0">
              <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{user?.name}</p>
              <p className="text-xs text-gray-500 dark:text-dark-text">Secrétaire</p>
            </div>
          )}
        </div>
