...
import { LayoutDashboard, Users, FileText, Calendar, ClipboardList, MessageCircle, Brain, UsersRound, Pill, LogOut, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    <SidebarShell>
      <div className="p-4 flex items-center justify-between border-b border-gray-100 dark:border-dark-border">
        {(!collapsed || isMobile) && (
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-medcare-blue to-medcare-purple flex items-center justify-center"><span className="text-white font-bold text-sm">M</span></div>
      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        {links.map(({ to, icon: Icon, label, end }) => (
          <NavLink key={to} to={to} end={end} onClick={handleNav}>
            {({ isActive }) => (
              <motion.div whileHover={{ x: 2 }} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${isActive ? 'bg-medcare-blue/10 text-medcare-blue' : 'text-gray-600 dark:text-dark-text hover:bg-gray-50 dark:hover:bg-dark-bg hover:text-gray-900 dark:hover:text-white'}`}>
                <Icon size={20} />
      ...
      <div className="p-3 border-t border-gray-100 dark:border-dark-border">
        <div className="flex items-center gap-3 px-3 py-2 mb-2">
          <div className="w-8 h-8 rounded-full bg-medcare-blue/20 flex items-center justify-center text-medcare-blue text-sm font-bold">{user?.name?.[0] || 'D'}</div>
          {(!collapsed || isMobile) && <div className="min-w-0"><p className="text-sm font-medium text-gray-900 dark:text-white truncate">{user?.name}</p><p className="text-xs text-gray-500 dark:text-dark-text">Médecin</p></div>}
        </div>
