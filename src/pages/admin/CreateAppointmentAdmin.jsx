...
import { Calendar, ArrowLeft, Stethoscope, AlertTriangle, CheckCircle, Users } from 'lucide-react';
import toast from 'react-hot-toast';
import api from '../../api/axios';

export default function CreateAppointmentAdmin() {
  const navigate = useNavigate();
  const [form, setForm] = useState({ patientId: '', doctorId: '', date: '', time: '09:00', type: 'consultation', urgencyDegree: 'normal', reason: '', symptoms: '', location: 'cabinet' });
  const [step, setStep] = useState(1);

  const { data: doctorsData, isLoading: doctorsLoading } = useQuery({
    queryKey: ['admin-doctors'],
    queryFn: async () => { const { data } = await api.get('/admin/doctors'); return data; }
  });

  const { data: patientsData, isLoading: patientsLoading } = useQuery({
    queryKey: ['admin-patients-list'],
    queryFn: async () => { const { data } = await api.get('/admin/patients?limit=200'); return data; }
  });

  const createMutation = useMutation({
    mutationFn: async (appointmentData) => { const { data } = await api.post('/appointments', appointmentData); return data; },
    onSuccess: () => { toast.success('Rendez-vous créé avec succès !'); navigate('/admin/patients'); },
      ...
      <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-white dark:bg-dark-card rounded-2xl border border-gray-100 dark:border-dark-border p-6">
        {step === 1 && (
          <div className="space-y-4">
            <h3 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2"><Users size={18} className="text-medcare-purple" /> Sélectionner un patient</h3>
            {patientsLoading ? (
              ...
              <p className="text-sm text-gray-400 text-center py-4">Aucun patient enregistré. Créez un patient d'abord.</p>
            ) : (
              <div className="space-y-2 max-h-80 overflow-y-auto">
                {patientList.map((p) => (
                  <button key={p._id} onClick={() => setForm({ ...form, patientId: p._id })} className={`w-full p-3 rounded-xl border-2 text-left transition-all ${form.patientId === p._id ? 'border-medcare-purple bg-medcare-purple/5' : 'border-gray-200 dark:border-dark-border hover:border-gray-300'}`}>
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            {doctorsLoading ? (
              ...
              <p className="text-sm text-gray-400 text-center py-4">Aucun médecin disponible.</p>
            ) : (
              <div className="space-y-2 max-h-80 overflow-y-auto">
                {doctorList.map((doc) => (
                  <button key={doc._id} onClick={() => setForm({ ...form, doctorId: doc._id })} className={`w-full p-4 rounded-xl border-2 text-left transition-all ${form.doctorId === doc._id ? 'border-medcare-purple bg-medcare-purple/5' : 'border-gray-200 dark:border-dark-border hover:border-gray-300'}`}>
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-medium text-gray-900 dark:text-white">Dr. {doc.name}</p>
                        <p className="text-sm text-gray-500">{doc.specializations?.[0]?.name || 'Médecin'}</p>
                      </div>
                      <span className={`text-xs px-2 py-1 rounded-full ${doc.currentStatus === 'disponible' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                        {doc.currentStatus === 'disponible' ? 'Disponible' : doc.currentStatus}
                      </span>
                    </div>
                  </button>
                ))}
