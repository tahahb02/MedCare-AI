        ...
        <div className="space-y-3">
          {upcoming.map(a => (
            <div key={a._id} className="bg-white dark:bg-dark-card rounded-2xl border border-gray-100 dark:border-dark-border p-4 flex flex-wrap items-center gap-3 md:gap-4">
              ...
              <div className="flex-1 min-w-0"><p className="font-medium text-gray-900 dark:text-white truncate">{a.doctorId?.name || 'Dr. Médecin'}</p><p className="text-sm text-gray-500 flex items-center gap-1"><Stethoscope size={12} /> {a.type || 'Consultation'} • {a.duration || 30}min</p><p className="text-xs text-gray-400 flex items-center gap-1 mt-1"><Clock size={10} /> {formatDateTime(a.date)}</p></div>
              <span className={`px-3 py-1 rounded-full text-xs font-medium flex-shrink-0 ${urgencyColor(a.urgencyDegree)}`}>{a.urgencyDegree}</span>
            </div>
          ))}
      ...
      <div>
        ...
        <div className="space-y-3">
          {past.slice(0, 10).map(a => (
            <div key={a._id} className="bg-white dark:bg-dark-card rounded-2xl border border-gray-100 dark:border-dark-border p-4 flex flex-wrap items-center gap-3 md:gap-4 opacity-75">
              ...
              <div className="flex-1 min-w-0"><p className="font-medium text-gray-900 dark:text-white truncate">{a.doctorId?.name || 'Dr. Médecin'}</p><p className="text-sm text-gray-500">{a.type || 'Consultation'}</p></div>
              <span className={`px-3 py-1 rounded-full text-xs font-medium flex-shrink-0 ${a.status === 'terminé' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>{a.status}</span>
            </div>
          ))}
